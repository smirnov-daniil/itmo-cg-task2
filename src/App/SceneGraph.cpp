...
void SceneNode::render(Camera * camera, const QVector3D & lightPos, const QVector3D & lightColor, OpenGLContextPtr context)
{
	if (!visible_)
		return;
	...
	{
		entity_->render(camera, lightPos, lightColor, context);
	}

	for (auto & child: children_)
	{
		child->render(camera, lightPos, lightColor, context);
	}
}
